...
Le projet **Lidl Drive** est une application web de type **Click & Collect** : un client peut parcourir un catalogue de produits, les ajouter à son panier, choisir un créneau de retrait, valider sa commande et aller la récupérer en magasin. L'application distingue trois types d'utilisateurs : le **client**, le **préparateur de commandes** (OPERATOR) et le **gérant de magasin** (MANAGER).

Ma contribution couvre l'ensemble de la **partie backend** : conception de l'architecture, développement des modules, mise en place de l'authentification, documentation de l'API, et connexion avec le frontend pour permettre la récupération des données. J'ai également participé à la **conception du schéma de base de données** en collaboration avec l'équipe, ainsi qu'à la mise en place du **Dockerfile** avec les autres membres du groupe.
...
- `id`, `permission_id` (FK), `last_name`, `first_name`, `email` (unique), `password` (hashé bcrypt), `phone`, `address`, `preferred_store_id` (nullable), `created_at`, `updated_at`

**`preparer`** — Préparateurs de commandes, rattachés à un magasin
- `id`, `permission_id` (FK), `store_id` (FK), `first_name`, `last_name_initials`, `work_email` (unique), `password` (hashé), `work_phone`, `created_at`, `updated_at`
...
L'inscription est réservée aux clients. Le mot de passe est **hashé avec bcrypt** avant toute persistance en base :
...
| **Sécurité mots de passe** | Tous les mots de passe sont hashés avec bcrypt avant toute persistance |
| **Unicité email** | Vérifié à l'inscription, levée d'un `ConflictException` (HTTP 409) si doublon |
| **Expiration des tokens** | 10 min pour ADMIN, 15 min pour les autres rôles |

---

## 11. Déploiement (contribution collective)
...
Ce projet m'a permis de concevoir et développer une **API REST complète en NestJS** dans un contexte e-commerce réaliste. Les compétences mobilisées couvrent :
